...
#include <stdio.h>
```
Linhas como essa são tipicamente colocadas no início do texto do programa, antes das funções. O caractere `#` deve ser o primeiro da linha. `stdio` inclui comandos de entrada e saída padrões (*standard io*).
...
Então, a sentença `printf("oi\nos");` vai caisar o envio de 5 códigos ao terminal: um para ele imprimir o símbolo `o`, outro para o `i`, outro para pular de linha, outro para imprimir o `o` e outro para imprimir o `s`.
